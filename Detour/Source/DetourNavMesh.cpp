...
inline bool overlapSlabs(const Real* amin, const Real* amax,
						 ...
						 const Real px, const Real py)
{
	...
	const Real minx = dtMax(amin[0]+px,bmin[0]+px);
	const Real maxx = dtMin(amax[0]-px,bmax[0]-px);
	if (minx > maxx)
		return false;
	
	// Check vertical overlap.
	const Real ad = (amax[1]-amin[1]) / (amax[0]-amin[0]);
	const Real ak = amin[1] - ad*amin[0];
	const Real bd = (bmax[1]-bmin[1]) / (bmax[0]-bmin[0]);
	const Real bk = bmin[1] - bd*bmin[0];
	const Real aminy = ad*minx + ak;
	const Real amaxy = ad*maxx + ak;
	const Real bminy = bd*minx + bk;
	const Real bmaxy = bd*maxx + bk;
	const Real dmin = bminy - aminy;
	const Real dmax = bmaxy - amaxy;
...
static Real getSlabCoord(const Real* va, const int side)
{
	if (side == 0 || side == 4)
		return va[0];
...
static void calcSlabEndPoints(const Real* va, const Real* vb, Real* bmin, Real* bmax, const int side)
{
	if (side == 0 || side == 4)
	{
		if (va[2] < vb[2])
...
dtNavMesh::dtNavMesh() :
	...
	m_nextFree(0),
	m_tiles(0),
	m_hasOffMesh ( true ) // https://github.com/recastnavigation/recastnavigation/pull/645/
{
#ifndef DT_POLYREF64
	m_saltBits = 0;
	m_tileBits = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////
int dtNavMesh::findConnectingPolys(const Real* va, const Real* vb,
								   const dtMeshTile* tile, int side,
								   dtPolyRef* con, Real* conarea, int maxcon) const
{
	if (!tile) return 0;
	
	Real amin[2], amax[2];
	calcSlabEndPoints(va, vb, amin, amax, side);
	const Real apos = getSlabCoord(va, side);
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			const Real* vc = &tile->verts[poly->verts[j]*3];
			const Real* vd = &tile->verts[poly->verts[(j+1) % nv]*3];
			const Real bpos = getSlabCoord(vc, side);
			
			// Segments are not close enough.
			if (dtAbs(apos-bpos) > Real ( 0.01f))
				continue;
			
			// Check if the segments touch.
			calcSlabEndPoints(vc,vd, bmin,bmax, side);
			
			if (!overlapSlabs(amin,amax, bmin,bmax, Real ( 0.01f), tile->header->walkableClimb)) continue;
...
//		unsigned short m = DT_EXT_LINK | (unsigned short)side;
		...
		for (int j = 0; j < nv; ++j)
		{
			...
			// Create new links
			const Real* va = &tile->verts[poly->verts[j]*3];
			const Real* vb = &tile->verts[poly->verts[(j+1) % nv]*3];
			dtPolyRef nei[4];
			Real neia[4*2];
			...
			for (int k = 0; k < nnei; ++k)
			{
				...
				if (idx != DT_NULL_LINK)
				{
					...
					if (dir == 0 || dir == 4)
					{
						Real tmin = (neia[k*2+0]-va[2]) / (vb[2]-va[2]);
						Real tmax = (neia[k*2+1]-va[2]) / (vb[2]-va[2]);
						if (tmin > tmax)
							dtSwap(tmin,tmax);
						link->bmin = (unsigned char)fixedmath::round(dtClamp(tmin, Real(0.0f), Real(1.0f))*Real(255.0f));
						link->bmax = (unsigned char)fixedmath::round(dtClamp(tmax, Real(0.0f), Real(1.0f))*Real(255.0f));
					}
					else if (dir == 2 || dir == 6)
					{
						Real tmin = (neia[k*2+0]-va[0]) / (vb[0]-va[0]);
						Real tmax = (neia[k*2+1]-va[0]) / (vb[0]-va[0]);
						if (tmin > tmax)
							dtSwap(tmin,tmax);
						link->bmin = (unsigned char)fixedmath::round(dtClamp(tmin, Real(0.0f), Real(1.0f))*Real(255.0f));
						link->bmax = (unsigned char)fixedmath::round(dtClamp(tmax, Real(0.0f), Real(1.0f))*Real(255.0f));
...
void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side)
{
	...
	for (int i = 0; i < target->header->offMeshConCount; ++i)
	{
		...
		const Real halfExtents[3] = { targetCon->rad, target->header->walkableClimb, targetCon->rad };
		
		// Find polygon to connect to.
		const Real* p = &targetCon->pos[3];
		Real nearestPt[3];
		...
		Real* v = &target->verts[targetPoly->verts[1]*3];
		dtVcopy(v, nearestPt);
...
void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->offMeshConCount; ++i)
	{
		...
		const Real halfExtents[3] = { con->rad, tile->header->walkableClimb, con->rad };
		
		// Find polygon to connect to.
		const Real* p = &con->pos[0]; // First vertex
		Real nearestPt[3];
		dtPolyRef ref = findNearestPolyInTile(tile, p, halfExtents, nearestPt);
		if (!ref) continue;
		// findNearestPoly may return too optimistic results, further check to make sure. 
		if (dtSqr(nearestPt[0]-p[0])+dtSqr(nearestPt[2]-p[2]) > dtSqr(con->rad))
			continue;
		// Make sure the location is on current mesh.
		Real* v = &tile->verts[poly->verts[0]*3];
		dtVcopy(v, nearestPt);
...
namespace
{
	...
	void closestPointOnDetailEdges(const dtMeshTile* tile, const dtPoly* poly, const Real* pos, Real* closest)
	{
		...
		for (int i = 0; i < pd->triCount; i++)
		{
			...
			for (int k = 0, j = 2; k < 3; j = k++)
			{
				...
				Real t;
				Real d = dtDistancePtSegSqr2D(pos, v[j], v[k], t);
				if (d < dmin)
				{
					dmin = d;
					tmin = t;
...
bool dtNavMesh::getPolyHeight(const dtMeshTile* tile, const dtPoly* poly, const Real* pos, Real* height) const
{
...
void dtNavMesh::closestPointOnPoly(dtPolyRef ref, const Real* pos, Real* closest, bool* posOverPoly) const
{
...
dtPolyRef dtNavMesh::findNearestPolyInTile(const dtMeshTile* tile,
										   const Real* center, const Real* halfExtents,
										   Real* nearestPt) const
{
	Real bmin[3], bmax[3];
	...
	dtPolyRef nearest = 0;
	Real nearestDistanceSqr = std::numeric_limits<Real>::max();
...
int dtNavMesh::queryPolygonsInTile(const dtMeshTile* tile, const Real* qmin, const Real* qmax,
							...
							dtTileRef lastRef, dtTileRef* result)
{
	...
	const int vertsSize = dtAlign4(sizeof(Real)*3*header->vertCount);
	const int polysSize = dtAlign4(sizeof(dtPoly)*header->polyCount);
	const int linksSize = dtAlign4(sizeof(dtLink)*(header->maxLinkCount));
	const int detailMeshesSize = dtAlign4(sizeof(dtPolyDetail)*header->detailMeshCount);
	const int detailVertsSize = dtAlign4(sizeof(Real)*3*header->detailVertCount);
	const int detailTrisSize = dtAlign4(sizeof(unsigned char)*4*header->detailTriCount);
	const int bvtreeSize = dtAlign4(sizeof(dtBVNode)*header->bvNodeCount);
	const int offMeshLinksSize = dtAlign4(sizeof(dtOffMeshConnection)*header->offMeshConCount);
	
	unsigned char* d = data + headerSize;
	tile->verts = dtGetThenAdvanceBufferPointer<Real>(d, vertsSize);
	...
	tile->detailVerts = dtGetThenAdvanceBufferPointer<Real>(d, detailVertsSize);
...
void dtNavMesh::connectGlobalOffMeshLinks(const dtMeshTile * tile, const dtMeshTile * target)
{
	...
	for (int i = 0; i < target->header->offMeshConCount; ++i)
	{
		...
		const Real ext[3] = { targetCon->rad, target->header->walkableClimb, targetCon->rad };
...
void dtNavMesh::calcTileLoc(const Real* pos, int* tx, int* ty) const
{
	*tx = (int)fixedmath::floor((pos[0]-m_orig[0]) / m_tileWidth);
	*ty = (int)fixedmath::floor((pos[2]-m_orig[2]) / m_tileHeight);
}
...
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, Real* startPos, Real* endPos) const
{
	unsigned int salt, it, ip;
